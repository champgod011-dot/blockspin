...
local antiLockBV = nil
local function removeAntiLockBV()
    if antiLockBV then
        pcall(function() antiLockBV:Destroy() end)
        antiLockBV = nil
    end
end

local function ensureAntiLockBV()
    local char = LocalPlayer.Character
    if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart")
    if not root then return end
    if antiLockBV and antiLockBV.Parent == root then return end
    removeAntiLockBV()
...
RunService.Heartbeat:Connect(function()
    if antiLockEnabled then
        ...
        if antiLockBV then
            ...
            local amount = 1500
            antiLockBV.Velocity = Vector3.new(
