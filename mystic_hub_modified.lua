...
--  High Jump
-- ══════════════════════════════════════════════════════════════
ContextActionService:BindAction("HighJump", function(_, state, input)
    ...
    if isJumpInput and state == Enum.UserInputState.Begin then
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    jumpConn = hum:GetPropertyChangedSignal("Jump"):Connect(function()
        if jumpPowerEnabled and hum.Jump then
            jumpHolding = true
        end
    end)
end)
